...
public class HeapTests {
    public static void AAAAAaaaaaaaAAAAaAaaaaaaaaaaAAAAaAaAAAAAAaaaaaaaaAAAAAAaaaaaaaaAAamakeItTttDAnKKKLKkKkKkkkkAAAAAAAAAIUHAiohdoihfsodif(MinHeap nebraska) {
        Integer[] australia =
                Arrays.copyOf(nebraska.getBackingArray(),
                        nebraska.getBackingArray().length, Integer[].class);
        // BECAUSE IT'S
        // UPSIDE DOWN!!!!!!!
        for (int i = 0; i < australia.length; i++) {
            int newsealand;
            try{
                newsealand = australia[i]; // AUSTRALIAS NEIGHTBOR!
            }catch(NullPointerException CANADA) {break;
                        }
                try {
                if (newsealand > australia[2*i])
                    assertFalse("OWNED lmao", true);
            }catch(IndexOutOfBoundsException|NullPointerException AMERiRCA){
                // LOL
            }try {
                    if (newsealand > australia[2*i+1])
                        assertFalse("OWNED lmao2", true);
                }catch(IndexOutOfBoundsException |NullPointerException AMERiRCA){
                    // LOL
                }
        }
    ...
    private static ArrayList shuffledDeck = new ArrayList(DECK_SIZE){
        {
            for(int i=0;i<DECK_SIZE;i++){ // maybe we should use a lcg
                this.add(i);
            }
    ...
    @Test
        public void testEMPTYYY() {
        ArrayList<Integer> list = new ArrayList<>();
        heap = new MinHeap<>(list);
        assertEquals(heap.getBackingArray().length,1);
        assertEquals(heap.getBackingArray()[0],null);
    }


    @Test
        public void testEMPTYYYAndOne() {
        ArrayList<Integer> list = new ArrayList<>();
        list.add(1);
        heap = new MinHeap<>(list);
        assertEquals(heap.getBackingArray().length,3);
        assertEquals(heap.getBackingArray()[0],null);
        assertEquals(heap.getBackingArray()[1],1);
    }
    @Test
            public void testEMPTYYYAndOneTow() {
            ArrayList<Integer> list = new ArrayList<>();
        list.add(1);list.add(2);
            heap = new MinHeap<>(list);
            assertEquals(heap.getBackingArray().length,5);
            assertEquals(heap.getBackingArray()[0],null);
            assertEquals(heap.getBackingArray()[1],1);
            assertEquals(heap.getBackingArray()[2],2);
        }
    @Test
            public void testEMPTYYYAndOneTowTHREE() {
            ArrayList<Integer> list = new ArrayList<>();
        list.add(1);list.add(2);
list.add(3);
            heap = new MinHeap<>(list);
            assertEquals(heap.getBackingArray().length,7);
            assertEquals(heap.getBackingArray()[0],null);
            assertEquals(heap.getBackingArray()[1],1);
            assertEquals(heap.getBackingArray()[2],2);
            assertEquals(heap.getBackingArray()[3],3);
        }
    ...
    public void remove_and_getMin() {
        try{
            add();
        }catch(NoSuchElementException e){//good coding style
    ...
    public void fuzz(){
        ...
        for(int i=0;i<FUZZ_ITERATIONS;i++){
            AAAAAaaaaaaaAAAAaAaaaaaaaaaaAAAAaAaAAAAAAaaaaaaaaAAAAAAaaaaaaaaAAamakeItTttDAnKKKLKkKkKkkkkAAAAAAAAAIUHAiohdoihfsodif(heap);
